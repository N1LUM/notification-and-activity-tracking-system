...
import (
	...
	"errors"
	"fmt"
	"notify-activity-tracking-system/internal/gateway/apperrors"
...
func (s *UserService) CreateUser(input user_context.CreateUserInputDTO) (*user_context2.User, error) {
	...
	createdUser, err := s.repo.Create(user_context2.User{
		Name:     input.Username,
...
func (s *UserService) GetAllUsers() ([]user_context2.User, error) {
...
func (s *UserService) GetByID(id uuid.UUID) (*user_context2.User, error) {
...
func (s *UserService) GetUserByUsername(username string) (*user_context2.User, error) {
...
func (s *UserService) Update(id uuid.UUID, input user_context.UpdateUserInputDTO) (*user_context2.User, error) {
	user, err := s.repo.GetByID(id)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, apperrors.ErrUserNotFound
